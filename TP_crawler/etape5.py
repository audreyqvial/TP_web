...
filename2 = path+"result_etape05.txt"
...
def parseTagContent(link):
    #pour l'étape 5, on n'utilise pas de regex, on recherche les liens de façon empirique
    #autrement dit j'ai déterminé de façon empirique les pages qui ne pouvaient pas être en relation avec les brevets
    not_interesting = ["/", "/register.html", "/search.html", 
    "http://research.freepatentsonline.com", "http://research.freepatentsonline.com/mpep", 
    "/tools-resources.html", "http://research.freepatentsonline.com/acclaimip", 
    "http://research.freepatentsonline.com/help", "/services.html", "/contact.html", 
    "http://research.freepatentsonline.com/help#advertising-on-site", 
    "https://twitter.com/FPOCommunity", "http://www.linkedin.com/groups/FPO-Community-4524797", 
    "/privacy.html", "http://www.sumobrainsolutions.com/", "http://research.freepatentsonline.com/help#pdf-help"]
    content = getURL(link)
    try:      
        soup = BeautifulSoup(content, 'html.parser')
        tags = soup('a')
        table_url = [tag.get('href') for tag in tags if tag.get('href') is not None]
        interesting_url = [url for url in table_url if url not in not_interesting]
        return tags, interesting_url
    except KeyError:
        return None, None
